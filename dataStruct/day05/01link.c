...
void link_rbegin(link_t *p_link) {
    p_link->p_cur = &(p_link->tail);   //为了保证下一个操作的节点是尾节点前面的最后一个有效节点
}
...
int link_prev(link_t *p_link, int *p_val) {
    ...
    p_link->p_cur = p_link->p_cur->p_prev;    //找到这次要操作的节点并记录到p_cur指针里
    if (p_link->p_cur == &(p_link->head)) {
        ...
        *p_val = p_link->p_cur->val;   //把节点里的数字传递给调用函数
        return 1;
    }
}
//颠倒数组里数字顺序的函数
void link_reverse(link_t *p_link) {
    node_t *p_tail = NULL;
    node_t *p_first1 = NULL, *p_mid1 = NULL, *p_last1 = NULL, *p_first2 = NULL, *p_mid2 = NULL, *p_last2 = NULL;
    if (p_link->head.p_next == &(p_link->tail)) {
        //如果头结点后面就是尾节点就
        //不能进行颠倒操作
        return ;
    }
    p_tail = p_link->tail.p_prev;    //p_tail指针记录原来的最后一个有效节点
    while (p_link->head.p_next != p_tail) {
        //头节点后面不是原来的最后一个
        //有效节点就表示还需要继续操作
        //每次循环把第一个有效节点摘出来
        //然后插到p_tail指针的后面
        //让第一组的三个指针指向链表最
        //前面的三个节点
        p_first1 = &(p_link->head);
        p_mid1 = p_first1->p_next;
        p_last1 = p_mid1->p_next;
        //把第一个有效节点从链表里摘出来
        p_first1->p_next = p_last1;
        p_last1->p_prev = p_first1;
        //让第二组的三个指针指向p_tail
        //开始的连续三个节点
        p_first2 = p_tail;
        p_mid2 = p_first2->p_next;
        p_last2 = p_mid2->p_next;
        //把摘出来的节点插在p_first2和p_mid2中间
        p_first2->p_next = p_mid1;
        p_mid1->p_next = p_mid2;
        p_mid2->p_prev = p_mid1;
        p_mid1->p_prev = p_first2;
    }
}
