...
int tree_size(const tree_t *p_tree) {
    ...
    cnt = tree_size(&(p_tree->p_node->left));  //计算左子树里的节点个数
    cnt += tree_size(&(p_tree->p_node->right));  //计算右子树里的节点个数
    return cnt + 1;
}
//计算树高度的函数
int tree_height(const tree_t *p_tree) {
    ...
    left_height = tree_height(&(p_tree->p_node->left));    //计算左子树的高度
    right_height = tree_height(&(p_tree->p_node->right));  //计算右子树的高度
    height = left_height > right_height ? left_height : right_height;   //得到两个子树最高的高度
    return height + 1;
}
